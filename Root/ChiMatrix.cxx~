...
ChiMatrix::ChiMatrix( string name ) : ChiMatrix()
{
  ...
  m_rand.SetSeed( m_eta1Bin*100+m_eta2Bin );
}
...
int ChiMatrix::Load( TFile *inFile, bool justTemplate ) {
  ...
  if ( !justTemplate ) {
    ...
    int i=1;
    while ( inFile->Get(  TString::Format( "%s_chi2FitNonConstVar_%i", m_name.c_str() , i ) ) ) {
      m_chi2FitNonConstVar.push_back(0);
      m_chi2FitNonConstVar.back() = (TH1D*) inFile->Get( TString::Format( "%s_chi2FitNonConstVar_%i", m_name.c_str() , i ) );
      i++;
    }
...
void ChiMatrix::FitChi2() {
  ...
  //Get the informations for the histograms definition
	...
	: m_chiMatrix->ProjectionY( TString::Format("%s_chi2FitNonConstVar_%d", m_name.c_str(), iConstVarBin ), iConstVarBin, iConstVarBin, "o");
      ...
      //Create the fit
      fitResult = FitHist( m_chi2FitNonConstVar.back(), !(m_setting->GetConstVarFit() == "SIGMA"), 0, 0 );
      if ( !fitResult->Status() ) {              
	double const *var = fitResult->GetParams();
	double alphaMin = var[2];
	double alphaErr = var[1];
	double chi2Min  = var[0];
  ...
  fitResult = FitHist( m_chi2FitConstVar , 1, 0, 0 );
  if ( !fitResult->Status() ) {  
    double const *var = fitResult->GetParams();
  ...
  if ( m_setting->GetDoScale() && m_setting->GetDoSmearing() ) {
    ...
    fitResult = m_corAngle->Fit( linearFit , "SQ", "RQ",  m_corAngle->GetXaxis()->GetBinLowEdge( max( 1, constVarBin -2 ) ), m_corAngle->GetXaxis()->GetBinUpEdge( min( m_corAngle->GetNbinsX(), constVarBin+2 ) ));
  ...
  cout << m_name << " scales : " << m_alpha << " " << m_sigma << endl;
...
void ChiMatrix::MakePlot( string path, string latexFileName ) {
  ...
  etaBins.insert( etaBins.begin(), -2.47 ); 
  ...
  expAlpha = ( m_setting->GetMode() == "1VAR" ) ? (alphaSimEta[m_eta1Bin] + alphaSimEta[m_eta2Bin])/2. : (alphaSimEta[m_eta1Bin] + alphaSimPt[m_eta2Bin])/2. ;
  expC = ( m_setting->GetMode() == "1VAR" ) ? sqrt((sigmaSimEta[m_eta1Bin]*sigmaSimEta[m_eta1Bin] + sigmaSimEta[m_eta2Bin]*sigmaSimEta[m_eta2Bin])/2.) : sqrt((sigmaSimEta[m_eta1Bin]*sigmaSimEta[m_eta1Bin] + sigmaSimPt[m_eta2Bin]*sigmaSimPt[m_eta2Bin])/2.); 
  ...
  latex << "MCEvents : " << m_MCZMass.front().front()->GetEntries() << "\\newline" << endl;
  ...
  if ( m_setting->GetDoScale() ) {
    ...
    legends.push_back( string(TString::Format("Template : alpha=%i",(int) ( m_scaleValues.front()*1e6)) ) );
    legends.push_back( string(TString::Format("Template : alpha=%i", (int) (m_scaleValues.back()*1e6)) ) );
  ...
  if ( m_setting->GetDoSmearing() ) {
    legends.clear();
    legends.push_back( "Data" );
    legends.push_back( string(TString::Format("Template : sigma=%i",(int) ( m_sigmaValues.front()*1e6) ) ));
    legends.push_back( string(TString::Format("Template : sigma=%i", (int) (m_sigmaValues.back()*1e6) )));
...
void ChiMatrix::FillScaleValues( int nTemplates) {
  ...
  for ( int i_alpha = 0; i_alpha <=alphaMaxBin; i_alpha++ ) {
    double scale = ( m_setting->GetDoScale() ) ? m_alphaMin + ( m_alphaMax - m_alphaMin ) /  alphaMaxBin * i_alpha : 0;
...
void ChiMatrix::OptimizeRanges( ) {
  if ( m_setting->GetDebug() ) cout << "ChiMatrix::OptimizeRanges() " << m_name << endl;
  ...
  for ( unsigned int iScale = 0; iScale < 2; iScale++ ) {
	...
	: m_chiMatrix->ProjectionX( m_name.c_str() + TString("histAlpha"), 1, 1, "o" );
      ...
      if ( !counter ) {
	TFitResultPtr fitResult = FitHist( histScale, iScale );
	if ( !fitResult->Status() ) {
	  double const *var = fitResult->GetParams();    
	  rangeMax = min( rangeMax, var[2] + m_setting->GetOptimizeRanges() * var[1] );
	  double minVal =  var[2] - m_setting->GetOptimizeRanges() * var[1];
	...
	//Dealing with low range too far
	if ( histScale->GetBinContent(1) - histScale->GetBinContent( minBin ) > m_setting->GetOptimizeRanges()*m_setting->GetOptimizeRanges() ) {
	  int bin = 1;
	  while ( bin<minBin-1 && histScale->GetBinContent( bin ) - histScale->GetBinContent(minBin) > m_setting->GetOptimizeRanges()*m_setting->GetOptimizeRanges()  ) bin++;
	  rangeMin = histScale->GetXaxis()->GetBinCenter( bin );
	  reachedPrecision = false;
	}
	
	//Dealing with high range too far
	if ( histScale->GetBinContent(histScale->GetNbinsX()) - histScale->GetBinContent( minBin ) > m_setting->GetOptimizeRanges()*m_setting->GetOptimizeRanges() ) {
	  int bin = histScale->GetNbinsX();
	  while ( bin>minBin+1 && histScale->GetBinContent( bin ) - histScale->GetBinContent(minBin) > m_setting->GetOptimizeRanges()*m_setting->GetOptimizeRanges()  ) bin--;
	  rangeMax = histScale->GetXaxis()->GetBinCenter( bin );
	  reachedPrecision = false;
	}


	//Dealing with low range too close
	if ( histScale->GetBinContent(1) - histScale->GetBinContent( minBin ) < m_setting->GetOptimizeRanges()*m_setting->GetOptimizeRanges()/2 && rangeMin != allowedRangeMin ) {
	  rangeMin = max( allowedRangeMin, rangeMin - histScale->GetXaxis()->GetBinWidth(minBin));
	  reachedPrecision = false;
	}
	
	//Dealing with high range too close
	if ( histScale->GetBinContent(histScale->GetNbinsX()) - histScale->GetBinContent( minBin ) < m_setting->GetOptimizeRanges()*m_setting->GetOptimizeRanges()/2 && rangeMax != allowedRangeMax ) {
	  rangeMax = min( allowedRangeMax, rangeMax + histScale->GetXaxis()->GetBinWidth(minBin) );
	  reachedPrecision = false;
	}
...
TFitResultPtr ChiMatrix::FitHist( TH1D* hist, unsigned int mode, double chiMinLow, double chiMinUp ) {
 ...
 //Create the fit
  ...
  int nFits=3;
  do {
    if ( nFits != 3 ) cout << "Fit retried : status " << fitResult->Status() << endl;
